  ...
  const connectToDevice = async () => {
    ...
    try {
      ...
      try {
        ...
        addLog('Bluetooth already on');
      }

      // ডিভাইস খোঁজা (এটিই স্বয়ংক্রিয়ভাবে পারমিশন প্রম্পট আনবে)
      addLog('Searching for Kama Clock...');
      const device = await BleClient.requestDevice({
        // আমরা এবার সার্ভিস ইউইউআইডি দিয়ে খুঁজব যাতে শুধু ঘড়িটাই আসে
        services: [UART_SERVICE_UUID]
      });

      addLog(`Connecting to ${device.name || 'Clock'}...`);
      ...
      console.error(error);
      addLog(`Status: ${error.message || 'No device selected'}`);
    } finally {
