...
export default function App() {
  ...
  const requestPermissions = async () => {
    ...
    try {
      ...
      try {
        ...
        addLog('Bluetooth enable failed/already on');
      }
  ...
  const connectToDevice = async () => {
    ...
    try {
      const device = await requestPermissions();
      ...
      if (error.message?.includes('permission') || error.message?.includes('denied')) {
        addLog('⚠️ Need "Nearby Devices" permission!');
      } else {
  ...
  const sendCommand = async () => {
    if (!device) {
      addLog('⚠️ Connect Bluetooth first!');
      return;
    }
      ...
      >
        ...
        <div className="p-8 pb-4 flex items-center justify-between relative z-10">
          ...
          <button 
            ...
            className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-500 ${device ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-slate-800 text-slate-400 hover:bg-slate-700 border border-slate-700'}`}
          >
            {isConnecting ? (
            ...
            <AnimatePresence mode="wait">
              {logs.length > 0 ? (
                ...
                <div className="flex items-center gap-2 text-slate-600">
                  ...
                  <span className="text-[10px] font-medium">Ready to connect</span>
                </div>
              )}
            </AnimatePresence>
          </div>
