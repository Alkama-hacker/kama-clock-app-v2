...
export default function App() {
  ...
  const connectToDevice = async () => {
    ...
    try {
      // অ্যান্ড্রয়েড ১২+ এর জন্য ব্লুটুথ অন করার রিকোয়েস্ট
      try {
        await BleClient.initialize();
        await BleClient.requestEnabled();
      } catch (e) {
        console.log('BLE already initialized or not supported');
      }
          ...
          <AnimatePresence mode="wait">
            {status && (
              <motion.div
                ...
                className="space-y-3"
              >
                <div className={`text-center text-sm font-medium p-3 rounded-xl ${status.includes('Error') || status.includes('Failed') || status.includes('⚠️') || status.includes('❌') ? 'bg-red-500/10 text-red-400' : 'bg-indigo-500/10 text-indigo-400'}`}>
                  {status}
                </div>
              </motion.div>
            )}
