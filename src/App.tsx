...
export default function App() {
  ...
  const connectToDevice = async () => {
    ...
    try {
      ...
      // Auto-sync current phone date and time
      const now = new Date();
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="relative group">
            ...
            <button
              onClick={sendCommand}
              disabled={!device}
              className={`relative w-full font-bold py-5 rounded-2xl flex items-center justify-center gap-3 transition-all duration-300 active:scale-95 ${
                device 
            ...
            >
              <Send className={`w-5 h-5 ${device ? 'animate-pulse' : ''}`} />
              <span className="tracking-wide uppercase text-sm">Sync to Clock</span>
            </button>
          </div>
        </div>
