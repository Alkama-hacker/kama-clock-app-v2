...
export default function App() {
  ...
  const connectToDevice = async () => {
    ...
    try {
      ...
      // এটি অ্যান্ড্রয়েড ১২+ এ ব্লুটুথ অন করার প্রম্পট আনবে
      try {
      ...
      // এটিই মেইন কমান্ড যা "Nearby Devices" পারমিশন প্রম্পট ট্রিগার করবে
      // আমরা এবার namePrefix ব্যবহার করছি যাতে সিস্টেম বাধ্য হয় প্রম্পট দেখাতে
      ...
      setStatus('System synced with phone time!');
      setTimeout(() => setStatus('Connected to Kama clock'), 3000);

    } catch (error: any) {
      console.error(error);
      if (error.message?.includes('permission') || error.message?.includes('denied')) {
        setStatus('⚠️ Please allow "Nearby Devices" permission in settings!');
      } else {
  ...
  const sendCommand = async () => {
    if (!device) {
      setStatus('⚠️ Please connect Bluetooth first!');
      return;
    }
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans flex flex-col items-center justify-center p-4">
      ...
      >
        ...
        <div className="p-8 space-y-8">
          ...
          <AnimatePresence mode="wait">
            {status && (
              <motion.div
                ...
                className={`text-center text-sm font-medium p-3 rounded-xl ${status.includes('⚠️') || status.includes('❌') ? 'bg-red-500/10 text-red-400' : 'bg-indigo-500/10 text-indigo-400'}`}
              >
                {status}
              </motion.div>
            )}
          ...
          <button
            onClick={sendCommand}
            className="w-full bg-indigo-500 hover:bg-indigo-600 text-white font-bold py-5 rounded-2xl flex items-center justify-center gap-3 transition-all shadow-xl shadow-indigo-500/20 active:scale-95"
          >
